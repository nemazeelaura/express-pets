...
exports.submitContact = async function (req, res, next) {
  ...
  try {
    ...
    const promise1 = transport.sendMail({
      ...
      <p>We appreciate your interest in ${doesPetExist.name} and one of our staff members will reach out shortly! Below is a copy of the message you sent us for your personal records:
...
exports.viewPetContacts = async (req, res) => {
  ...
  const contacts = await contactsCollection.find({ petId: new ObjectId(req.params.id) }).toArray() /* from "":id" we added to url in router file*/
  res.render("pet-contacts", { contacts, pet }) /*same name propery and value variable (contacts)so don't need : and []*/
}